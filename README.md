...
```yaml
name: Abharan Gupta
role: Software Development Engineer II @ Kapture CX
education: B.E. Computer Science — Chitkara University (CGPA: 9.89)
location: India

current_focus:
  - Leading end-to-end product feature delivery at scale
  - Building bulk migration microservices (Salesforce, Zendesk, Zoho → Kapture)
  - Designing reliable, zero-downtime enterprise onboarding systems

highlights:
  - Built real-time WebSocket pipeline → 10,000+ events/sec, 90% latency reduction
  - Architected Social Media microservice integrating 6+ platforms
  - Migrated file storage AWS S3 → GCP → 50% lower data latency
  - Resolved 200+ P0 incidents, reducing MTTR by 50%

hobbies: ["🏸 Badminton", "🎮 Gaming", "🚴 Riding"]
```
...
```text
🟣 Kapture CX  •  SDE II                     Oct 2025 – Present
   ├── Leading end-to-end global product feature delivery
   ├── Built bulk migration microservice (Salesforce, Zendesk, Zoho → Kapture)
   ├── Reduced client workflow time by 70% with Excel-based bulk upload tooling
   └── Migrated file storage to GCS — 50% lower data latency

🔵 Kapture CX  •  Software Developer          Sep 2023 – Sep 2025
   ├── Built real-time notification pipeline: WebSocket + STOMP (10K+ events/sec)
   ├── Architected Social Media integration microservice (6+ platforms)
   ├── Owned REST APIs & resolved 200+ P0 incidents (50% MTTR reduction)
   └── Earned Top Performer recognition — 2 consecutive quarters
```
